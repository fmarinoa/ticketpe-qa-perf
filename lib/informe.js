...
// Uso en un caso: `summaryTrendStats: STATS` en options + `export const handleSummary = resumirCon(options, 'título');`
...
const condicion = (scenarios) =>
  Object.entries(scenarios)
    .map(([n, s]) => `${n}: ${[s.executor, (s.vus || s.maxVUs) && `${s.vus || s.maxVUs} VUs`, s.duration, s.iterations && `${s.iterations} iteraciones`, s.rate && `${s.rate}/${s.timeUnit}`].filter(Boolean).join(' · ')}`)
    .join(' | ');

// En handleSummary k6 ya reemplazó `options` por su objeto interno (sin `scenarios` legibles):
// se copia en la fase init, cuando todavía es el objeto JS declarado por el caso.
export const resumirCon = (options, titulo) => {
  const copia = JSON.parse(JSON.stringify(options));
  return (data) => informe(data, copia, titulo);
};
